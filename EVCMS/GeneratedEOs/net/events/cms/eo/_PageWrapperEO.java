...
// DO NOT EDIT.  Make changes to PageWrapperEO.java instead.
...
public abstract class _PageWrapperEO extends net.events.cms.eo.EVCMSGenericRecord {
	private static Logger log = Logger.getLogger( _PageWrapperEO.class );
     ...
     */
    public static final String ENTITY_NAME = "PageWrapperEO";

	/**
	 * Standard constructor
	 */	
    public _PageWrapperEO() {
        super();
    }
	...
	/**
	 * Create a "PageWrapperEO" object with all required values
	 */
	public static PageWrapperEO createPageWrapperEO(EOEditingContext editingContext, String componentTemplate, String name, net.events.cms.eo.Client client, net.events.cms.eo.Site site) {
		if (log.isDebugEnabled()) log.debug ("Creating object: PageWrapperEO");
		PageWrapperEO eoObject = (PageWrapperEO)EOUtilities.createAndInsertInstance(editingContext, _PageWrapperEO.ENTITY_NAME);
 	...
 	/**
 	 * Fetch all "PageWrapperEO" objects from the database. Be careful with that function
 	 * as it might return a lot of objects
 	 */
	public static NSArray fetchAllObjects(EOEditingContext _editingContext) {
		if (log.isDebugEnabled()) log.debug ("Fetching all objects for entity 'PageWrapperEO'");
		return _PageWrapperEO.fetchAllObjects(_editingContext, null);
	}
	...
	public static NSArray fetchAllObjects(EOEditingContext _editingContext, NSArray _sortOrderings) {
		if (log.isDebugEnabled()) log.debug ("Fetching all objects for entity 'PageWrapperEO' with sortOrderings " + _sortOrderings);
		return _PageWrapperEO.fetchObjectsWithQualifier(_editingContext, null, _sortOrderings);
	}

	/**
	 * Fetches all "PageWrapperEO" objects matching value for Key
	 */
	public static NSArray fetchObjectsWithKeyAndValue(EOEditingContext _editingContext, String _keyName, Object _value) {
		return _PageWrapperEO.fetchObjectsWithQualifier(_editingContext, new EOKeyValueQualifier(_keyName, EOQualifier.QualifierOperatorEqual, _value), null);
	}

	/**
	 * Fetches all "PageWrapperEO" objects matching the given qualifier
	 */
	public static NSArray fetchObjectsWithQualifier(EOEditingContext _editingContext, EOQualifier _qualifier, NSArray _sortOrderings) {
		if (log.isDebugEnabled()) log.debug ("Fetching all objects for entity 'PageWrapperEO' with qualifier " + _qualifier + "and sortOrderings " + _sortOrderings);
		EOFetchSpecification fetchSpec = new EOFetchSpecification(_PageWrapperEO.ENTITY_NAME, _qualifier, _sortOrderings);
 	 ...
 	 */
	public static PageWrapperEO fetchPageWrapperEOWithKeyAndValue(EOEditingContext _editingContext, String _keyName, Object _value) {
		return _PageWrapperEO.fetchPageWrapperEOWithQualifier(_editingContext, new EOKeyValueQualifier(_keyName, EOQualifier.QualifierOperatorEqual, _value));
	}
 	 ...
 	 */
 	public static PageWrapperEO fetchPageWrapperEOWithQualifier(EOEditingContext _editingContext, EOQualifier _qualifier) {
		if (log.isDebugEnabled()) log.debug ("Fetching 'PageWrapperEO' with qualifier: " + _qualifier);
		NSArray eoObjects = _PageWrapperEO.fetchObjectsWithQualifier(_editingContext, _qualifier, null);
		PageWrapperEO eoObject;
		int count = eoObjects.count();
		if (count == 0) {
			eoObject = null;
  		}
		else if (count == 1) {
			eoObject = (PageWrapperEO)eoObjects.objectAtIndex(0);
		}
		else {
			throw new IllegalStateException("There was more than one PageWrapperEO that matched the qualifier '" + _qualifier + "'.");
		}
		return eoObject;
	}
 	 ...
 	 */
	public static PageWrapperEO fetchRequiredPageWrapperEOWithKeyAndValue(EOEditingContext _editingContext, String _keyName, Object _value) {
		return _PageWrapperEO.fetchRequiredPageWrapperEOWithQualifier(_editingContext, new EOKeyValueQualifier(_keyName, EOQualifier.QualifierOperatorEqual, _value));
	}
 
 	/**
 	 * Fetch exactly one object for value for key. Might throw an Exception if none or more than one object was found!
 	 */
	public static PageWrapperEO fetchRequiredPageWrapperEOWithQualifier(EOEditingContext _editingContext, EOQualifier _qualifier) {
		if (log.isDebugEnabled()) log.debug ("Fetching required 'PageWrapperEO' with qualifier: " + _qualifier);
		PageWrapperEO eoObject = _PageWrapperEO.fetchPageWrapperEOWithQualifier(_editingContext, _qualifier);
		if (eoObject == null) {
			throw new NoSuchElementException("There was no PageWrapperEO that matched the qualifier '" + _qualifier + "'.");
   		}
		return eoObject;
	}
 	 ...
 	 */
	public PageWrapperEO localInstanceOfPageWrapperEO(EOEditingContext _editingContext) {
		if (log.isDebugEnabled()) log.debug ("Returning local instance of 'PageWrapperEO': " + this.toString());
		return (PageWrapperEO)EOUtilities.localInstanceOfObject(_editingContext, this);
	}
 
 	/** 
 	 * Returns a local object or null; this works for null objects as wel
 	 */
	public static PageWrapperEO localInstanceOfPageWrapperEO(EOEditingContext _editingContext, PageWrapperEO _eo) {
		if (_eo != null) {
			if (log.isDebugEnabled()) log.debug ("Returning local instance of 'PageWrapperEO': " + _eo.toString());
		}
		return (_eo == null) ? null : (PageWrapperEO)EOUtilities.localInstanceOfObject(_editingContext, _eo);
	}
